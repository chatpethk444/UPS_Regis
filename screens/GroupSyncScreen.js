...
export default function GroupSyncScreen({ student, setView }) {
  ...
  useEffect(() => {
    if (groupInfo) {
      ...
      const myMember = groupInfo.members?.find(
  ...
  const handleJoinGroup = async () => {
    if (!groupCode.trim())
      return showModal("แจ้งเตือน", "กรุณากรอกรหัสกลุ่ม", "warning"); // 🌟 ใช้ Modal
    setLoading(true);
    try {
    ...
    showModal(
      ...
      async () => {
        ...
        try {
          ...
          setTimeout(
            () => showModal("ไม่สามารถ Sync ได้", e.message, "error"),
            500,
          );
        } finally {
          setLoading(false);
  ...
  const myMemberData = groupInfo?.members?.find(
    (m) => String(m.student_id) === String(student.student_id),
  );
  const approvedMembers =
    groupInfo?.members?.filter((m) => m.status === "APPROVED") || [];
  ...
  const needsSync =
    lastSync &&
    approvedMembers.some((m) => new Date(m.joined_at) > new Date(lastSync));
    ...
    return (
      <LinearGradient colors={["#FFDAE4", "#FFF8F8"]} style={styles.container}>
        ...
        >
          ...
          <Text style={{ marginTop: 15, color: "#a73355", fontWeight: "bold" }}>
        ...
        >
          {!hasGroup ? (
            ...
            <View style={styles.pendingContainer}>
              ...
              <View style={styles.pendingCard}>
                ...
                <Text style={styles.pendingTitle}>
                  รอหัวหน้ากลุ่มอนุมัติ...
                </Text>
                <Text style={styles.pendingDesc}>
                  คุณได้ส่งคำขอเข้าร่วมกลุ่มแล้ว
                  กรุณารอให้หัวหน้ากลุ่มกดยืนยันคำขอของคุณ
                </Text>
            ...
            <View>
              ...
              >
                <Text style={{ color: "#FDEEF4", fontSize: 12 }}>
                  GROUP CODE
                </Text>
                <Text
                  style={{
              ...
              {needsSync && isMyLeader && (
                <View style={styles.syncWarning}>
                  ...
                  <Text style={styles.syncWarningText}>
                    มีสมาชิกใหม่! กรุณา Sync ตะกร้าอีกครั้ง
                  </Text>
                </View>
              )}

              <Text style={styles.sectionTitle}>
                สมาชิก ({approvedMembers.length}/5)
              </Text>
              {groupInfo.members.map((m, i) => (
                <View key={i} style={styles.memberCard}>
                  ...
                  >
                    {m.status === "APPROVED" ? (
                      m.student_id === groupInfo.group.leader_id ? (
                      ...
                      isMyLeader && (
                        <View style={{ flexDirection: "row", gap: 5 }}>
                          <TouchableOpacity
                            onPress={() =>
                              handleApprove(m.student_id, "REJECT")
                            }
                          ...
                          <TouchableOpacity
                            onPress={() =>
                              handleApprove(m.student_id, "APPROVE")
                            }
                    ...
                    {isMyLeader &&
                      ...
                      m.student_id !== student.student_id && (
                        <TouchableOpacity
                          onPress={() => handleApprove(m.student_id, "REJECT")}
              ...
              <Text style={styles.sectionTitle}>
                ตารางเรียนกลุ่ม (จากตะกร้าหัวหน้า)
              </Text>
              {!showLeaderCart ? (
                <View style={styles.lockedCart}>
                  <MaterialIcons name="lock" size={40} color="#ccc" />
                  <Text style={styles.lockedCartText}>
                    รอหัวหน้า Sync ตะกร้า
                  </Text>
                </View>
              ) : (
                <View style={styles.gridOuterContainer}>
                  <ScrollView horizontal showsHorizontalScrollIndicator={true}>
                    <View
                      style={{
                        width: TOTAL_GRID_WIDTH + DAY_COLUMN_WIDTH + 20,
                      }}
                    >
                          ...
                          <Text
                            ...
                            style={[
                              styles.timeLabel,
                              {
                                position: "absolute",
                                left:
                                  i * ONE_HOUR_WIDTH + DAY_COLUMN_WIDTH - 10,
                                width: 20,
                                textAlign: "center",
                              },
                            ]}
                        ...
                        (dayKey) => (
                          <View key={dayKey} style={styles.dayRow}>
                            <View style={styles.dayLabelContainer}>
                              <Text style={styles.dayTextSmall}>
                                {DAY_MAP[dayKey]}
                              </Text>
                            </View>
                            <View
                            ...
                            >
                              {cartData
                                .filter((c) => c.day.includes(DAY_MAP[dayKey]))
                                .map((item, idx) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <Text
                                        ...
                                        numberOfLines={1}
                                      >
                                        {item.course_code} ({item.section_type})
                                      </Text>
                                      <Text
                  ...
                  <View key={idx} style={styles.courseCardMini}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.miniCode}>
                        {item.course_code} Sec {item.section} (
                        {item.section_type})
                      </Text>
                      ...
                      <Text style={styles.miniSeats}>
                        ที่นั่ง: {item.enrolled_seats}/{item.max_seats} (ว่าง{" "}
                        {item.max_seats - item.enrolled_seats})
                      </Text>
                    </View>
                  </View>
                ))}
              ...
              <View style={styles.footerActions}>
                {isMyLeader ? (
                  <>
                    <TouchableOpacity
                      style={[styles.mainBtn, !allReady && { opacity: 0.5 }]}
                      disabled={!allReady}
