...
import { getWaitlistStatusAPI, confirmWaitlistSeatAPI, cancelWaitlistAPI } from '../api';

const { width } = Dimensions.get("window");

// 🌟 1. สร้าง Custom Alert Modal ให้ UI สวยงาม
const CustomAlert = ({ visible, title, message, type, onConfirm, onCancel }) => {
  if (!visible) return null;

  const getIcon = () => {
    switch (type) {
      case "success": return <MaterialIcons name="check-circle" size={50} color="#4CAF50" />;
      case "warning": return <MaterialIcons name="warning" size={50} color="#FF9800" />;
      case "danger": return <MaterialIcons name="error" size={50} color="#F44336" />;
      default: return <MaterialIcons name="info" size={50} color="#2196F3" />;
    }
  };
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalBtnGroup}>
              ...
              <TouchableOpacity style={{ flex: 1 }} onPress={onConfirm}>
                <LinearGradient 
                  colors={type === 'danger' ? ["#F44336", "#D32F2F"] : type === 'warning' ? ["#FF9800", "#F57C00"] : ["#4CAF50", "#2E7D32"]} 
                  style={styles.modalBtnConfirm}
                >
                  <Text style={styles.modalBtnTextConfirm}>{onCancel ? "ยืนยัน" : "ตกลง"}</Text>
                </LinearGradient>
              </TouchableOpacity>
            </View>
          </View>
...
const WaitlistCard = ({ 
  ...
  useEffect(() => {
    ...
    const calculateTimeLeft = () => {
      const now = new Date();
      const validDateStr = allocated_at.endsWith("Z") ? allocated_at : allocated_at + "Z";
    ...
    <View style={styles.card}>
      <View style={styles.header}>
        <View style={{ flex: 1, paddingRight: 10 }}>
          ...
          <Text style={{ fontSize: 14, fontWeight: 'bold', color: "#514345", marginTop: 2 }}>
            {course_name}
          </Text>
          <Text style={styles.sectionInfo}>กลุ่ม {section_number} ({typeLabel})</Text>
        </View>
        <View style={[styles.statusBadge, { backgroundColor: getStatusColor() + "15", alignSelf: 'flex-start' }]}>
          <Text style={[styles.statusText, { color: getStatusColor() }]}>
             {status === "PENDING" ? "กำลังรอคิว" 
             : status === "ALLOCATED" ? "ได้สิทธิ์แล้ว" 
             : status === "CONFIRMED" ? "ยืนยันสำเร็จ" 
             : "หมดเวลา/สละสิทธิ์"}
          </Text>
        </View>
      </View>

      <View style={{ backgroundColor: '#f9f9f9', padding: 10, borderRadius: 8, marginTop: 10 }}>
        <Text style={{ fontSize: 12, color: '#837375', marginBottom: 4 }}>
          <MaterialIcons name="schedule" size={14} color="#837375" /> เรียน: {schedule}
        </Text>
        <Text style={{ fontSize: 12, color: '#837375', marginBottom: 4 }}>
        ...
        {created_at && (
          <Text style={{ fontSize: 12, color: '#a73355' }}>
            <MaterialIcons name="history" size={14} color="#a73355" /> กดคิวเมื่อ: {formatJoinedDate(created_at)}
          </Text>
        )}
      </View>
        ...
        {status === "PENDING" ? (
          ...
          <View style={styles.allocatedContainer}>
            ...
            <View style={styles.btnGroup}>
              ...
              <TouchableOpacity onPress={onConfirm} activeOpacity={0.8} style={styles.btnConfirmContainer}>
                <LinearGradient colors={["#4CAF50", "#2E7D32"]} style={styles.btnConfirm}>
          ...
          <View style={[styles.pendingContainer, { backgroundColor: "#E8F5E9", justifyContent: "center" }]}>
            <MaterialIcons name="check-circle" size={24} color="#4CAF50" />
            <Text style={[styles.queueLabel, { color: "#2E7D32", flex: 0, marginLeft: 8, fontWeight: "bold" }]}>
              คุณได้ยืนยันสิทธิ์วิชานี้แล้ว
            </Text>
          </View>
        ) : (
          <View style={styles.expiredContainer}>
            <Text style={styles.expiredText}>คุณไม่ได้ยืนยันสิทธิ์ในเวลาที่กำหนด หรือสละสิทธิ์ไปแล้ว</Text>
          </View>
        )}
      </View>
    </View>
...
const WaitlistScreen = ({ setView, student }) => {
  ...
  const [alertConfig, setAlertConfig] = useState({
    visible: false, title: "", message: "", type: "info", onConfirm: null, onCancel: null
  });
  ...
  const fetchWaitlistData = useCallback(async (isSilent = false) => {
    ...
    try {
      ...
      if (!isSilent) showAlert("ข้อผิดพลาด", error.message || "ไม่สามารถดึงข้อมูลรายการต่อคิวได้", "danger", closeAlert);
    } finally {
    ...
    showAlert(
      ...
      async () => {
        ...
        try {
          ...
          setTimeout(() => {
            showAlert("สำเร็จ 🎉", `เพิ่มวิชา ${course_code} ลงในตารางเรียนเรียบร้อยแล้ว!`, "success", closeAlert);
            fetchWaitlistData(false); 
          }, 500); // หน่วงเวลาให้ UI ปิด Modal แรกก่อนเปิดอันใหม่
        } catch (error) {
          setTimeout(() => showAlert("ไม่สามารถยืนยันสิทธิ์ได้", error.message, "danger", closeAlert), 500);
        }
      },
      closeAlert
    );
    ...
    showAlert(
      ...
      async () => {
        ...
        try {
          ...
          setTimeout(() => {
            showAlert("สำเร็จ", "ยกเลิกคิวเรียบร้อยแล้ว", "success", closeAlert);
            fetchWaitlistData(false);
          }, 500);
        } catch (error) {
          setTimeout(() => showAlert("ข้อผิดพลาด", error.message || "ไม่สามารถยกเลิกคิวได้", "danger", closeAlert), 500);
        }
      },
      closeAlert
    );
    ...
    <LinearGradient colors={["#FFDAE4", "#FFF8F8"]} style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.screenHeader}>
          ...
          <TouchableOpacity onPress={() => fetchWaitlistData(false)} style={{ padding: 8 }}>
          ...
          {loading ? (
            ...
            waitlists.map((item) => (
              <WaitlistCard 
                ...
                onCancel={() => handleCancelSeat(item.id, item.course_id, item.status === "PENDING")}
              />
            ))
          ) : (
...
const styles = StyleSheet.create({
  ...
  backBtn: {
    ...
    justifyContent: "center", alignItems: "center", elevation: 2, paddingLeft: 8,
  },
  ...
  pendingContainer: {
    flexDirection: "row", alignItems: "center", justifyContent: "space-between",
    backgroundColor: "#FFF9C4", padding: 12, borderRadius: 12,
  },
  ...
  timerBox: {
    ...
    backgroundColor: "#ffebee", padding: 8, borderRadius: 8,
  },
  timerText: { fontSize: 13, color: "#514345" },
  countdown: { fontSize: 16, fontWeight: "bold", color: "#F44336", fontFamily: "monospace" },
  ...
  btnCancel: { backgroundColor: "#f5f5f5", borderWidth: 1, borderColor: "#e0e0e0" },
  btnTextCancel: { color: "#F44336", fontWeight: "bold" },
  btnConfirmContainer: { flex: 1.5 },
  btnConfirm: { height: 48, borderRadius: 12, justifyContent: "center", alignItems: "center" },
  ...
  modalTitle: { fontSize: 20, fontWeight: "bold", color: "#333", marginTop: 16, marginBottom: 8, textAlign: "center" },
  modalMessage: { fontSize: 14, color: "#666", textAlign: "center", marginBottom: 24, lineHeight: 22 },
